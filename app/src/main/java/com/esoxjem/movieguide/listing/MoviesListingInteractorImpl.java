...
class MoviesListingInteractorImpl implements MoviesListingInteractor
{
    ...
    {
        return Observable.fromCallable(() -> getMovieList(page));
    }


    private List<Movie> getMovieList(int page) throws IOException, JSONException
    {
        int selectedOption = sortingOptionStore.getSelectedOption();
        if (selectedOption == SortType.MOST_POPULAR.getValue())
        {
            return fetchMovieList(Api.GET_POPULAR_MOVIES + "&page=" + page);
        } else if (selectedOption == SortType.HIGHEST_RATED.getValue())
        {
            return fetchMovieList(Api.GET_HIGHEST_RATED_MOVIES + "&page=" + page);
        } else
        {
